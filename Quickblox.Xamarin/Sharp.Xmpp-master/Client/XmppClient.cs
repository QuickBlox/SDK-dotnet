	...
	public class XmppClient : IDisposable {
		...
		public XmppClient(string hostname, string username, string password,
			int port = 5222, bool tls = true) {
				im = new XmppIm(hostname, username, password, port, tls, null);
		...
		public XmppClient(string hostname, int port = 5222, bool tls = true) {
				im = new XmppIm(hostname, port, tls, null);
				LoadExtensions();
		}
		...
		public Message SendMessage(Jid to, string messageId, string body, string extraParamets = null, string subject = null,
        ...
        public void SetvCardAvatar(Stream fileStream)
        {
            AssertValid();
            ...
            try
            {
                vcardAvatars.SetAvatar(fileStream);
            }           
        ...
        public void GetvCardAvatar(Jid jid, Action<string> callback)
        {
            AssertValid();
            vcardAvatars.RequestAvatar(jid, callback);

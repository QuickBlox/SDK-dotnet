using System;
using System.Globalization;
using System.Xml;
using System.Xml.Linq;

namespace Xmpp.Core
{
    /// <summary>
    /// Represents a XmppMessage XML stanza.
    /// </summary>
    public class Message : Stanza
    {
        /// <summary>
        /// Initializes a new instance of the XmppMessage class.
        /// </summary>
        /// <param name="to">The JID of the intended recipient for the stanza.</param>
        /// <param name="from">The JID of the sender.</param>
        /// <param name="data">The content of the stanza.</param>
        /// <param name="id">The ID of the XmppMessage stanza.</param>
        /// <param name="language">The language of the XML character data of
        /// the stanza.</param>
        public Message(Jid to = null, Jid from = null, XElement data = null,
            string id = null, CultureInfo language = null)
            : base(null, to, from, id, language, data)
        {
        }

        /// <summary>
        /// Initializes a new instance of the XmppMessage class from the specified
        /// Xml element.
        /// </summary>
        /// <param name="element">An Xml element representing an XmppMessage stanza.</param>
        /// <exception cref="ArgumentNullException">The element parameter is null.</exception>
        public Message(XElement element)
            : base(element)
        {
        }
    }
}
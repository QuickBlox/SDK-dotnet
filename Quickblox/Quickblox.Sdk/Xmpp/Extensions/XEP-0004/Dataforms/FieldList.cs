...
namespace Xmpp.Extensions.Dataforms
{
    ...
    public class FieldList : IEnumerable<DataField>, IReadOnlyCollection<DataField>
    {
        ...
        public void Remove(string name)
        {
            if (name == null)
                return;
            XElement e = GetFieldElementByName(name);
            if (e != null)
        ...
        public void Clear()
        {
            ...
            foreach (XElement e in element.Descendants("field"))
        ...
        public FieldList(XElement element, bool readOnly = false)
        {
        ...
        private IList<XElement> GetFieldElements()
        {
            IList<XElement> elems = new List<XElement>();
            foreach (XElement e in element.Descendants("field"))
            {
                //if (e.ParentNode == element)
                if (e.Parent == element)
        ...
        private IList<DataField> GetFields()
        {
            ...
            foreach (XElement e in GetFieldElements())
        ...
        /// <exception cref="ArgumentNullException">The name parameter is
        /// null.</exception>
        private XElement GetFieldElementByName(string name)
        {
            name.ThrowIfNull("name");
            foreach (XElement e in GetFieldElements())
            {
        ...
        private DataFieldType? GetDataFieldType(XElement element)
        {
            try
            {

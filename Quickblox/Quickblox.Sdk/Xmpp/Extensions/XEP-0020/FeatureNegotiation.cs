...
namespace Xmpp.Extensions
{
    ...
    internal class FeatureNegotiation : XmppExtension
    {
        ...
        /// null.</exception>
        public static XElement Create(DataForm form)
        {
        ...
        /// data.</exception>
        public static DataForm Parse(XElement feature)
        {
            feature.ThrowIfNull("feature");
            if (feature.Name != "feature" || feature.GetDefaultNamespace().NamespaceName !=
                "http://jabber.org/protocol/feature-neg" || feature.Element("x") == null)
            {
                throw new ArgumentException("Invalid XML 'feature' element.");
            }
            return DataFormFactory.Create(feature.Element("x"));
        }

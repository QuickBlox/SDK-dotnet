...
namespace Xmpp.Extensions
{
        ...
        IOutputFilter<Presence>
    {
        ...
        {
            var c = stanza.Data.Element("c");
            if (c == null || c.GetDefaultNamespace().NamespaceName != "http://jabber.org/protocol/caps")
                return false;
        ...
        public void Output(Presence stanza)
        {
            var c = Xml.Element("c", "http://jabber.org/protocol/caps")
                ...
                .SetAttribute("node", nodeUri)
                .SetAttribute("ver", GenerateVerificationString());

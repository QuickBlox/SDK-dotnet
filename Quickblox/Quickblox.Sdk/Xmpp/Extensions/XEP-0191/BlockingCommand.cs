    ...
    internal class BlockingCommand : XmppExtension, IInputFilter<Iq>
    {
        ...
        public bool Input(Iq stanza)
        {
            if (stanza.Type != IqType.Set)
                return false;
            if (stanza.Data.Element("block") == null && stanza.Data.Element("unblock") == null)
                return false;
            bool isBlock = stanza.Data.Element("block") != null;
            var elem = isBlock ? stanza.Data.Element("block") : stanza.Data.Element("unblock");
            // Element could be part of another extension, so examine namespace.
            if (elem.GetDefaultNamespace().NamespaceName != "urn:xmpp:blocking")
                return false;
            // Acknowledge the "push" request.
            im.IqResult(stanza);
            // Raise events for blocked/unblocked items.
            foreach (XElement item in elem.Descendants("item"))
            {
                try
                {

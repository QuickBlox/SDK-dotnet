namespace System.Security.Cryptography
{
    internal class SHA1Internal
    {
        ...
        {
            this._H = new uint[5];
            this._ProcessingBuffer = new byte[BLOCK_SIZE_BYTES];
        ...
        public void HashCore(byte[] rgb, int ibStart, int cbSize)
        {
            ...
            {
                if (cbSize < (BLOCK_SIZE_BYTES - this._ProcessingBufferCount))
                {
                    ...
                    i = (BLOCK_SIZE_BYTES - this._ProcessingBufferCount);
            ...
            {
                System.Buffer.BlockCopy(rgb, cbSize - cbSize % BLOCK_SIZE_BYTES + ibStart, this._ProcessingBuffer, 0, cbSize % BLOCK_SIZE_BYTES);
                this._ProcessingBufferCount = cbSize % BLOCK_SIZE_BYTES;
            }
        }
        ...
        public byte[] HashFinal()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    hash[i * 4 + j] = (byte)(this._H[i] >> (8 * (3 - j)));
                }
            }
        ...
        {
            ulong total = this.count + (ulong)inputCount;
            ...
            byte[] fooBuffer = (length == 64) ? this._ProcessingBuffer : new byte[length];
            ...
            ulong size = total << 3;
            this.AddLength(size, fooBuffer, inputCount + paddingSize);

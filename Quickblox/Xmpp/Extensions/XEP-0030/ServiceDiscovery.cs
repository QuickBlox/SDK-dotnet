...
namespace Sharp.Xmpp.Extensions
{
    ...
    internal class ServiceDiscovery : XmppExtension, IInputFilter<Iq>
    {
        ...
        public bool Input(Iq stanza)
        {
            ...
            if (query.GetDefaultNamespace().NamespaceName == "http://jabber.org/protocol/disco#items")
            {
                im.IqResult(stanza, Xml.Element("query",
                    "http://jabber.org/protocol/disco#items"));
                return true;
            }
            if (query.GetDefaultNamespace().NamespaceName != "http://jabber.org/protocol/disco#info")
                return false;
            // Construct the response which consists of an 'identity' element and a set of
            // 'feature' elements.
            var ident = Xml.SetAttribute(Xml.Element("identity").SetAttribute("category", this.Identity.Category)
                    .SetAttribute("type", this.Identity.Type), "name", Identity.Name);
            var xml = Xml.Element("query", "http://jabber.org/protocol/disco#info")
                .Child(ident);
            foreach (string xmlns in CompileFeatureSet())
                xml.Child(Xml.Element("feature").SetAttribute("var", xmlns));
        ...
        private IEnumerable<Extension> QueryFeatures(Jid jid)
        {
            ...
            if (query == null || query.GetDefaultNamespace().NamespaceName != "http://jabber.org/protocol/disco#info")
                throw new NotSupportedException("Erroneous response: " + iq);
            ISet<string> ns = new HashSet<string>();
            foreach (XElement e in query.Descendants("feature"))
        ...
        private IEnumerable<Identity> QueryIdentities(Jid jid)
        {
            ...
            foreach (XElement e in query.Descendants("identity"))
            {
        ...
        private IEnumerable<Item> QueryItems(Jid jid)
        {
            ...
            // Parse the result.
            var query = iq.Data.Element("query");
            if (query == null || query.GetDefaultNamespace().NamespaceName != "http://jabber.org/protocol/disco#items")
                throw new NotSupportedException("Erroneous response: " + iq);
            ISet<Item> items = new HashSet<Item>();
            foreach (XElement e in query.Descendants("item"))
            {

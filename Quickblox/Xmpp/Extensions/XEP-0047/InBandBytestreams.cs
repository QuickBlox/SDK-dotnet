    ...
    internal class InBandBytestreams : XmppExtension, IInputFilter<Iq>, IDataStream
    {
        ...
        public bool Input(Iq stanza)
        {
            if (stanza.Type != IqType.Set)
                return false;
            var e = stanza.Data.Element("open");
            if (e == null)
                e = stanza.Data.Element("close");
            if (e == null)
                e = stanza.Data.Element("data");
            if (e == null || e.GetDefaultNamespace().NamespaceName != "http://jabber.org/protocol/ibb")
                return false;
        ...
        private void Open(string sessionId, Iq stanza)
        {
            ...
            string s = stanza.Data.Element("open").GetAttribute("stanza");
        ...
        private void Data(string sessionId, Iq stanza)
        {
            sessionId.ThrowIfNull("sessionId");
            stanza.ThrowIfNull("stanza");
            var data = stanza.Data.Element("data");
            if (data == null)
        ...
        private void OpenStream(Jid to, string sessionId)
        {
            ...
            var open = Xml.Element("open", "http://jabber.org/protocol/ibb")
                .SetAttribute("block-size", blockSize.ToString())
                .SetAttribute("sid", sessionId)
                .SetAttribute("stanza", "iq");

...
namespace Sharp.Xmpp.Extensions
{
    ...
    internal class InBandRegistration : XmppExtension
    {
        ...
        public void Register(RegistrationCallback callback)
        {
            ...
            bool xdata = query.Element("x") != null;
            if (xdata)
                form = DataFormFactory.Create(query.Element("x")) as RequestForm;
        ...
        /// element.</returns>
        private RequestForm CreateDataForm(XElement query)
        {
            string instructions = query.Element("instructions") != null ?
                query.Element("instructions").Value : null;
            RequestForm form = new RequestForm(null, instructions);
            foreach (XElement child in query.Nodes())
            {
                if (child.Name == "instructions")
                    continue;
                form.Fields.Add(new TextField(child.Name.LocalName, true));
            }
            return form;
        }
    }

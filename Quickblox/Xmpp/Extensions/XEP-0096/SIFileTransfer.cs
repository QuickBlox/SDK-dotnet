...
namespace Sharp.Xmpp.Extensions
{
    ...
    internal class SIFileTransfer : XmppExtension
    {
            ...
            string description = null, Action<bool, FileTransfer> cb = null)
        {
            ...
            //return InitiateFileTransfer(to, stream, info.Name, info.Length, description, cb);
            return InitiateFileTransfer(to, new MemoryStream(), "TODO added", 1, description, cb);
        }
        ...
        private XElement OnStreamInitiationRequest(Jid from, XElement si)
        {
            try
            {
                string method = SelectStreamMethod(si.Element("feature"));
                ...
                var file = si.Element("file");
                string desc = file.Element("desc") != null ? file.Element("desc").Value : null,
                ...
                //TODO: add reading from file
                //SISession session = new SISession(sid, File.OpenWrite(savePath), size, true, from, im.Jid, im.GetExtension(method) as IDataStream);

                SISession session = new SISession(sid, new MemoryStream(), size, true, from, im.Jid, im.GetExtension(method) as IDataStream);
        ...
        /// supported.</exception>
        private string SelectStreamMethod(XElement feature)
        {

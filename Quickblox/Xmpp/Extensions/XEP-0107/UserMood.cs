...
namespace Xmpp.Extensions
{
    ...
    internal class UserMood : XmppExtension
    {
        ...
        /// event.</param>
        private void onMood(Jid jid, XElement item)
        {
            if (item == null || item.Element("mood") == null)
                return;
            var moodElement = item.Element("mood");
            Mood? mood = null;
            ...
            else
            {
                ...
                foreach (var v in Enum.GetValues(typeof(Mood)))
                {
                    ...
                    if (moodElement.Element(s) != null)
                        mood = (Mood)v;
                }
            }
            string text = moodElement.Element("text") != null ?
                moodElement.Element("text").Value : null;

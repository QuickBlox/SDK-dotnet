...
namespace Xmpp.Extensions
{
    ...
    internal class Pep : XmppExtension, IInputFilter<Im.Message>
    {
        ...
        /// </summary>
        private IDictionary<string, Action<Jid, XElement>> callbacks =
            new Dictionary<string, Action<Jid, XElement>>();
        ...
        public bool Input(Im.Message stanza)
        {
            ...
            {
                callbacks[nodeId].Invoke(stanza.From, items.Element("item"));
                return true;
            }
            return false;
        }
        ...
        public void Publish(string node, string itemId = null, params XElement[] data)
        {
            ...
            var xml = Xml.Element("pubsub", "http://jabber.org/protocol/pubsub")
                .Child(Xml.Element("publish").SetAttribute("node", node));
            if (data != null)
            {
                ...
                if (!item.IsEmpty)
                    xml.Element("publish").Child(item);
            }
        ...
        public void Subscribe(string node, Action<Jid, XElement> cb)
        {
            node.ThrowIfNull("node");
            cb.ThrowIfNull("cb");
        ...
        /// <exception cref="XmppException">The server returned invalid data or another
        /// unspecified XMPP error occurred.</exception>
        public IEnumerable<XElement> RetrieveItems(Jid jid, string node)
        {
            jid.ThrowIfNull("jid");
            node.ThrowIfNull("node");
            var xml = Xml.Element("pubsub", "http://jabber.org/protocol/pubsub")
                .Child(Xml.Element("items").SetAttribute("node", node));
            Iq iq = im.IqRequest(IqType.Get, jid, im.Jid, xml);
            if (iq.Type == IqType.Error)
                throw Util.ExceptionFromError(iq, "The items could not be retrieved.");
            var pubsub = iq.Data.Element("pubsub");
            if (pubsub == null || pubsub.GetDefaultNamespace().NamespaceName != "http://jabber.org/protocol/pubsub")
                throw new XmppException("Expected 'pubsub' element: " + iq);
            var items = pubsub.Element("items");
            ...
            foreach (XElement e in items.Descendants("item"))
                set.Add(e);
            return set;
        }
        ...
        /// <exception cref="XmppException">The server returned invalid data or another
        /// unspecified XMPP error occurred.</exception>
        public XElement RetrieveItem(Jid jid, string node, string itemId)
        {
            jid.ThrowIfNull("jid");
            node.ThrowIfNull("node");
            itemId.ThrowIfNull("itemId");
            var xml = Xml.Element("pubsub", "http://jabber.org/protocol/pubsub").Child(
                Xml.Element("items").SetAttribute("node", node).Child(
                Xml.Element("item").SetAttribute("id", itemId))
            );
            Iq iq = im.IqRequest(IqType.Get, jid, im.Jid, xml);
            if (iq.Type == IqType.Error)
            ...
            if (items == null || items.GetAttribute("node") != node)
                throw new XmppException("Expected 'items' element: " + iq);
            if (items.Element("item") == null || items.Element("item").GetAttribute("id") != itemId)
                throw new XmppException("Expected 'item' element: " + items.ToXmlString());
            return items.Element("item");
        }

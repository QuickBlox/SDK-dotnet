    ...
    internal class BitsOfBinary : XmppExtension, IInputFilter<Iq>
    {
        ...
        public bool Input(Iq stanza)
        {
            if (stanza.Type != IqType.Get)
                return false;
            var e = stanza.Data.Element("data");
            if (e == null || e.GetDefaultNamespace().NamespaceName != "urn:xmpp:bob")
                return false;
            string cid = e.GetAttribute("cid");
            if (cache.ContainsKey(cid))
            {
                BobData b = cache[cid];
                var data = Xml.SetAttribute(Xml.Element("data", "urn:xmpp:bob").SetAttribute("cid", cid), "type", b.Type)
